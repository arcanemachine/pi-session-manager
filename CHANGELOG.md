...
- `/session-manager configure|status` user command using a native `On`/`Off`
  selector with the current authorization state first/default and a transient
  status notification.
...
- Focused unit tests for authorization lifetime, command parsing/current-state
  selector behavior, and stable tool registration.
