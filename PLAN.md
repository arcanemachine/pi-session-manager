...
- **Task 2 — Tmux adapter and inventory:** Accepted after implementation, Architect/coordinator review, and independent package-local format, typecheck, test, and diff verification.
- **Task 3 — Create tool:** Accepted after implementation, Architect/coordinator review, and independent package-local format, typecheck, test, and diff verification.
...
- **Task 7 — Full verification and user acceptance:** In progress during live UAT. Steps 1–3 and 4–22 passed; separate-process authorization isolation also passed. The corrected readable list output passed live retest for retained exited worker 1, including exit status, signal availability, and exit time; dead-only close then removed that worker. The approved authorization UX—native `/session-manager configure` selector with the current state first/default, retained `status`, and no direct `on`/`off` arguments—passed Architect review, independent format/typecheck/full-test/diff verification, and live both-direction testing. Ordinary close rejected live worker 3, force close without confirmation rejected before mutation, explicitly approved force-close removed worker 3, both close operations protected attached worker 2, manual reviewer deletion removed its final window, an untagged diagnostic session remained untouched and ignored, reload retained authorization, and final disablement denied tools. Step 23 is blocked: the current default tmux state differs from the preserved baseline with an additional `practorium-smoke` session and additional window differences; these unrelated changes cannot be attributed from observation alone. Obtain user clarification before final acceptance.
...
- **Outstanding Task 1 live verification:** A launched worker Pi started disabled while the Manager remained enabled, including disabled tool denial. Enabled-state `configure` ordering and real-TUI `/reload` authorization retention passed; disabled-state `configure` ordering and deliberate transition to `On` remain pending.
...
- **Outstanding Task 5 live verification:** Dead-only close, live-close/confirmation-gate checks, force-close success, attached-human protection, manual deletion, and untagged-object handling passed. Final-session disappearance remains pending.
