...
- **Task 7 — Full verification and user acceptance:** In progress. Live acceptance passed creation, multi-fleet topology, attachment, authorization isolation, and bounded no-focus view through step 10. Step 11 exposed a blocker: `pi_fleet_list` retained the exited pane and its structured details but its LLM-readable content omitted the plan-required exit status/signal/time and other required per-instance fields. A bounded rendering/test correction is assigned to `pi-session-manager-worker`; live UAT is paused with worker instance 1 retained for retest.
